...
const Page = ({ params }) => {
  ...
  const addSession = async () => {
    if (form.day === null || form.hour === "" || form.min === "" || form.AmPm === null || form.grade === null) {
      ...
      const response = await Axios.put("/api/sessions/", {
        sessionId: form._id,
        day: form.day,
        hour: form.hour,
        min: form.min,
  ...
  useEffect(() => {
    ...
    Axios.get("/api/grades/").then((response) => {
      ...
      setGrades(grades)
      Axios.get(`/api/sessions/${params.sessionId}`).then((response) => {
        const session = response.data
        const gradeIndex = grades.find((grade) => grade._id === session.grade._id)
        ...
        <Body>
          ...
          <RightSide title={"SESSIONS"} active={"addSessions"}>
            <ClassForm >
              ...
              <Btn submitForm={addSession} btn={{ type: "button", className: "btn btn-primary", value: "تعديل" }} />
            </ClassForm>
          </RightSide>
        </Body>
        <Footer />
